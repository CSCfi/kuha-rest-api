...
func mapInsertRaceJPInput(in InsertRaceJPInput) (fis.InsertRaceJPClean, error) {
	var rd, sed, recv, vald, lup *time.Time
	var h1, h2, h3, c1, c2, c3 *time.Time
	var err error

	if in.Racedate != nil {
		rd, err = utils.ParseDatePtr(in.Racedate)
		if err != nil {
			return fis.InsertRaceJPClean{}, err
		}
	}
	if in.Starteventdate != nil {
		sed, err = utils.ParseDatePtr(in.Starteventdate)
	...
	if in.Hloc1 != nil {
		h1, err = utils.ParseTimestampPtr(in.Hloc1)
		if err != nil {
			return fis.InsertRaceJPClean{}, err
		}
	}
	if in.Hloc2 != nil {
		h2, err = utils.ParseTimestampPtr(in.Hloc2)
		if err != nil {
			return fis.InsertRaceJPClean{}, err
		}
	}
	if in.Hloc3 != nil {
		h3, err = utils.ParseTimestampPtr(in.Hloc3)
		if err != nil {
			return fis.InsertRaceJPClean{}, err
		}
	}
	if in.Hcet1 != nil {
		c1, err = utils.ParseTimestampPtr(in.Hcet1)
		if err != nil {
			return fis.InsertRaceJPClean{}, err
		}
	}
	if in.Hcet2 != nil {
		c2, err = utils.ParseTimestampPtr(in.Hcet2)
		if err != nil {
			return fis.InsertRaceJPClean{}, err
		}
	}
	if in.Hcet3 != nil {
		c3, err = utils.ParseTimestampPtr(in.Hcet3)
		if err != nil {
			return fis.InsertRaceJPClean{}, err
		}
	}

	return fis.InsertRaceJPClean{
		Raceid:            in.Raceid,
		Eventid:           in.Eventid,
		Seasoncode:        in.Seasoncode,
		Racecodex:         in.Racecodex,
...
func mapUpdateRaceJPInput(in UpdateRaceJPInput) (fis.UpdateRaceJPClean, error) {
	clean, err := mapInsertRaceJPInput(InsertRaceJPInput(in))
	return fis.UpdateRaceJPClean(clean), err
}
...
func FISRaceJPFullFromSqlc(row fissqlc.ARacejp) FISRaceJPFullResponse {
	var raceDateStr, startEventStr, receivedStr, validStr, lastUpdateStr *string
	var h1, h2, h3, c1, c2, c3 *string
	...
	if row.Receiveddate.Valid {
		receivedStr = utils.FormatTimestampPtr(row.Receiveddate)
	}
	if row.Validdate.Valid {
		validStr = utils.FormatTimestampPtr(row.Validdate)
	}
	if row.Lastupdate.Valid {
		lastUpdateStr = utils.FormatTimestampPtr(row.Lastupdate)
	}
	if row.Hloc1.Valid {
		h1 = utils.FormatTimestampPtr(row.Hloc1)
	}
	if row.Hloc2.Valid {
		h2 = utils.FormatTimestampPtr(row.Hloc2)
	}
	if row.Hloc3.Valid {
		h3 = utils.FormatTimestampPtr(row.Hloc3)
	}
	if row.Hcet1.Valid {
		c1 = utils.FormatTimestampPtr(row.Hcet1)
	}
	if row.Hcet2.Valid {
		c2 = utils.FormatTimestampPtr(row.Hcet2)
	}
	if row.Hcet3.Valid {
		c3 = utils.FormatTimestampPtr(row.Hcet3)
	}

	return FISRaceJPFullResponse{
		Raceid:            row.Raceid,
		Eventid:           utils.Int32PtrOrNil(row.Eventid),
		Seasoncode:        utils.Int32PtrOrNil(row.Seasoncode),
		Racecodex:         utils.Int32PtrOrNil(row.Racecodex),
		...
		SkipWcsl:          utils.Int32PtrOrNil(row.SkipWcsl),
		Lastupdate:        lastUpdateStr,
		Validforowg:       utils.StringPtrOrNil(row.Validforowg),
	}
}
...
// Insert / Update payloads (strings for dates/timestamps, parsed to time.Time)
...
func mapInsertRaceNKInput(in InsertRaceNKInput) (fis.InsertRaceNKClean, error) {
	var (
		rd, sed, recv, vald, lup *time.Time
		hl1, hl2, hl3            *time.Time
		hc1, hc2, hc3            *time.Time
		err                      error
	)
	...
	if in.Racedate != nil {
		rd, err = utils.ParseDatePtr(in.Racedate)
		if err != nil {
			return fis.InsertRaceNKClean{}, err
		}
	}
	if in.Starteventdate != nil {
		sed, err = utils.ParseDatePtr(in.Starteventdate)
	...
	if in.Hloc1 != nil {
		hl1, err = utils.ParseTimestampPtr(in.Hloc1)
		if err != nil {
			return fis.InsertRaceNKClean{}, err
		}
	}
	if in.Hloc2 != nil {
		hl2, err = utils.ParseTimestampPtr(in.Hloc2)
		if err != nil {
			return fis.InsertRaceNKClean{}, err
		}
	}
	if in.Hloc3 != nil {
		hl3, err = utils.ParseTimestampPtr(in.Hloc3)
		if err != nil {
			return fis.InsertRaceNKClean{}, err
		}
	}
	if in.Hcet1 != nil {
		hc1, err = utils.ParseTimestampPtr(in.Hcet1)
		if err != nil {
			return fis.InsertRaceNKClean{}, err
		}
	}
	if in.Hcet2 != nil {
		hc2, err = utils.ParseTimestampPtr(in.Hcet2)
		if err != nil {
			return fis.InsertRaceNKClean{}, err
		}
	}
	if in.Hcet3 != nil {
		hc3, err = utils.ParseTimestampPtr(in.Hcet3)
		if err != nil {
			return fis.InsertRaceNKClean{}, err
		}
	}

	return fis.InsertRaceNKClean{
		Raceid:            in.Raceid,
		Eventid:           in.Eventid,
		Seasoncode:        in.Seasoncode,
		Racecodex:         in.Racecodex,
...
func mapUpdateRaceNKInput(in UpdateRaceNKInput) (fis.UpdateRaceNKClean, error) {
	clean, err := mapInsertRaceNKInput(InsertRaceNKInput(in))
	return fis.UpdateRaceNKClean(clean), err
}

// Full response mapping from sqlc ARacenk
...
func FISRaceNKFullFromSqlc(row fissqlc.ARacenk) FISRaceNKFullResponse {
	...
	if row.Hloc1.Valid {
		hl1 = utils.FormatTimestampPtr(row.Hloc1)
	}
	if row.Hloc2.Valid {
		hl2 = utils.FormatTimestampPtr(row.Hloc2)
	}
	if row.Hloc3.Valid {
		hl3 = utils.FormatTimestampPtr(row.Hloc3)
	}
	if row.Hcet1.Valid {
		hc1 = utils.FormatTimestampPtr(row.Hcet1)
	}
	if row.Hcet2.Valid {
		hc2 = utils.FormatTimestampPtr(row.Hcet2)
	}
	if row.Hcet3.Valid {
		hc3 = utils.FormatTimestampPtr(row.Hcet3)
	}

	return FISRaceNKFullResponse{
		Raceid:            row.Raceid,
		Eventid:           utils.Int32PtrOrNil(row.Eventid),
		Seasoncode:        utils.Int32PtrOrNil(row.Seasoncode),
		Racecodex:         utils.Int32PtrOrNil(row.Racecodex),
